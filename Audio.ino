...
RGB LED colour state
WiFi connected: green
WebSocket connected: cyan
WebSocket disconnected / error: magenta
Recording: red
Sending: orange
Waiting for server response: yellow
Playback playing: light blue/teal
Idle/ready: blue
 */
...
#include <ESPmDNS.h>
#include "base64.h"
#include "WaitingMusic.hpp"
// --- RGB LED Control (WS2812 via RMT) ---
#include "driver/rmt.h"
#ifndef BUILTIN_RGBLED_PIN
#define BUILTIN_RGBLED_PIN 48
#endif
#define RGB_NR_OF_LEDS 1
#define RGB_BITS (24 * RGB_NR_OF_LEDS)

rmt_data_t rgb_led_data[RGB_BITS];

// color order for WS2812 is GRB
void setRgbLed(uint8_t r, uint8_t g, uint8_t b) {
  int color[3] = {g, r, b};
  int i = 0;
  for (int led = 0; led < RGB_NR_OF_LEDS; led++) {
    for (int col = 0; col < 3; col++) {
      for (int bit = 0; bit < 8; bit++) {
        if ((color[col] & (1 << (7 - bit)))) {
          rgb_led_data[i].level0 = 1;
          rgb_led_data[i].duration0 = 8;
          rgb_led_data[i].level1 = 0;
          rgb_led_data[i].duration1 = 4;
        } else {
          rgb_led_data[i].level0 = 1;
          rgb_led_data[i].duration0 = 4;
          rgb_led_data[i].level1 = 0;
          rgb_led_data[i].duration1 = 8;
        }
        i++;
      }
    }
  }
  // send - using the helper rmtWrite from the example environment
  rmtWrite(BUILTIN_RGBLED_PIN, rgb_led_data, RGB_BITS, RMT_WAIT_FOR_EVER);
}

// Audio Stream Queue System
#define MAX_AUDIO_STREAMS 5
#define STREAM_BUFFER_SIZE 524288  // 512KB per stream
...
public:
    bool startNewStream(unsigned long streamId) {
        Serial.printf("🔵 startNewStream called: streamId=%lu, count=%d, nextStreamIndex=%d\n", 
            streamId, count, nextStreamIndex);
        
        if (count >= MAX_AUDIO_STREAMS) {
            Serial.printf("⚠️ Queue full! count=%d, MAX=%d\n", count, MAX_AUDIO_STREAMS);
            return false;
        }
        
        // Use nextStreamIndex for allocation
        if (streams[nextStreamIndex].data) {
            Serial.printf("🗑️ Freeing old stream %d data\n", nextStreamIndex);
            heap_caps_free(streams[nextStreamIndex].data);
            streams[nextStreamIndex].data = nullptr;
        }
        
        size_t free_psram = heap_caps_get_free_size(MALLOC_CAP_SPIRAM);
        Serial.printf("💾 Allocating %d bytes (free PSRAM: %d bytes)\n", STREAM_BUFFER_SIZE, free_psram);
        
        streams[nextStreamIndex].data = (uint8_t*)heap_caps_malloc(STREAM_BUFFER_SIZE, MALLOC_CAP_SPIRAM);
        if (!streams[nextStreamIndex].data) {
            Serial.printf("❌ Stream allocation failed! Tried to allocate %d bytes from PSRAM\n", STREAM_BUFFER_SIZE);
            return false;
        }
        ...
        Serial.printf("✅ Started stream %d (ID: %lu) at %p\n", nextStreamIndex, streamId, streams[nextStreamIndex].data);
        
        // Set writeIndex to the stream we just allocated
        writeIndex = nextStreamIndex;
        
        count++;
        Serial.printf("📊 Queue state: count=%d, writeIndex=%d, readIndex=%d\n", count, writeIndex, readIndex);
    ...
    bool addData(const uint8_t* data, size_t len) {
        if (count == 0 || !streams[writeIndex].data) {
            Serial.printf("⚠️ Cannot add data: count=%d, writeIndex=%d, data=%p\n", 
                count, writeIndex, streams[writeIndex].data);
            return false;
        }
        
        if (streams[writeIndex].size + len > streams[writeIndex].capacity) {
            Serial.printf("⚠Stream %d buffer full! (size=%d, capacity=%d, trying to add=%d)\n", 
                writeIndex, streams[writeIndex].size, streams[writeIndex].capacity, len);
    ...
    bool completeStreamById(unsigned long streamId) {
        if (count == 0) return false;
        
        // Find the stream with this ID and mark it complete
        for (int i = 0; i < MAX_AUDIO_STREAMS; i++) {
            if (streams[i].data && streams[i].streamId == streamId) {
                streams[i].complete = true;
                Serial.printf("✅ Stream %d (ID: %lu) marked complete: %d bytes\n", i, streamId, streams[i].size);
                return true;
            }
        }
        Serial.printf("⚠️ Stream ID %lu not found to mark complete\n", streamId);
        return false;
    }
...
// Waiting music URL (16kHz, 16-bit, mono WAV)
const char* WAIT_MUSIC_URL = "https://raw.githubusercontent.com/InX-de-sign/yp-codes/refs/heads/main/Du_16k_mono_s16.wav";
...
// Playback state variables (moved from static to global for proper reset)
size_t playbackOffset = 44;
bool headerSkipped = false;
unsigned long finishTime = 0;
unsigned long drainDelay = 0;
...
void webSocketEvent(WStype_t type, uint8_t * payload, size_t length) {
  switch(type) {
    case WStype_DISCONNECTED:
      ...
      // Indicate WebSocket disconnected (magenta)
      setRgbLed(32,0,32);
      break;
    ...
    case WStype_TEXT:
      {
        ...
        Serial.printf("🔍 Parsed msgType: '%s'\n", msgType.c_str());
        ...
        if (msgType == "registered") {
          ...
          String text = doc["text"].as<String>();
            ...
            // Prefer server-provided stream id (phrase) if present
            unsigned long sid = 0;
            if (doc.containsKey("phrase")) sid = (unsigned long)doc["phrase"].as<unsigned long>();
            else if (doc.containsKey("stream_id")) sid = (unsigned long)doc["stream_id"].as<unsigned long>();
            else { streamCounter++; sid = streamCounter; }

            if (audioQueue.startNewStream(sid)) {
                Serial.printf("✅ Stream started (ID: %lu)\n", sid);
            } else {
                Serial.println("❌ Failed to start stream - queue full");
            }

            // stop waiting music when server audio arrives
            waitingMusic.stop();
            digitalWrite(I2S_SD_SPK, LOW);
            // Server audio incoming -> set playing color (teal)
            setRgbLed(0,20,20);
          }
            else if (msgType == "audio_complete" || msgType == "tts_complete") {
              // Prefer a server-provided id to mark the correct stream complete
              unsigned long completeId = 0;
              if (doc.containsKey("phrase")) completeId = (unsigned long)doc["phrase"].as<unsigned long>();
              else if (doc.containsKey("stream_id")) completeId = (unsigned long)doc["stream_id"].as<unsigned long>();

              bool marked = false;
              if (completeId != 0) {
                marked = audioQueue.completeStreamById(completeId);
                Serial.printf("Stream ID %lu completion: %s\n", completeId, marked ? "SUCCESS" : "NOT FOUND");
              }
              if (!marked) {
                // Fallback: mark the current write stream complete
                audioQueue.completeCurrentStream();
                Serial.println("⚠️ Using fallback: marked current write stream complete");
              }

              // Always try to start playback when completion message received
              if (!isPlayingAudio && audioQueue.hasStreams()) {
                Serial.println("▶️ Starting playback from audio_complete...");
                isPlayingAudio = true;
                // Playback starting -> light blue
                setRgbLed(0,16,32);
              }
            }
    ...
    case WStype_BIN:
        {
            // AUTO-ALLOCATE: If no stream exists, create one automatically
            if (!audioQueue.hasStreams()) {
                Serial.println("⚠️ Binary data arrived before audio_start - auto-allocating stream");
                streamCounter++;
                if (!audioQueue.startNewStream(streamCounter)) {
                    Serial.println("❌ Auto-allocation failed!");
                    break;
                }
            }
...
void send_audio_to_server() {
  ...
  // Start waiting music loop while waiting for server response
  if (waitingMusic.isLoaded()) {
    waitingMusic.start();
...
void play_response_audio() {
    ...
    if (!headerSkipped) {
        ...
        drainDelay = (samples * 1000 / SAMPLE_RATE) + 2000;  // Add 2 second margin for I2S buffer
        Serial.printf("🕐 Initial drain delay: %lu ms\n", drainDelay);
    ...
    if (playbackOffset < current->size) {
        ...
        if (finishTime == 0) {
          // Recalculate drainDelay using the current (final) stream size
          size_t audioDataBytesFinal = current->size - 44;
          size_t samplesFinal = audioDataBytesFinal / 2;
          drainDelay = (samplesFinal * 1000 / SAMPLE_RATE) + 2000;  // 2 second margin for I2S buffer
          finishTime = millis();
          Serial.printf("⏳ Recalculated drain: %lu ms for %d bytes\n", drainDelay, current->size);
        }
        
        // Wait for calculated drain time
        if (millis() - finishTime > drainDelay) {
          // Advance if stream is marked complete, OR if we've waited too long (5 second timeout)
          bool shouldAdvance = current->complete;
          unsigned long waitTime = millis() - finishTime;
          
          if (!shouldAdvance && waitTime > 5000) {
            Serial.printf("⚠️ Timeout after %lu ms - forcing advance (not marked complete)\n", waitTime);
            shouldAdvance = true;
          }
          
          if (shouldAdvance) {
            Serial.printf("✅ Stream playback done: %d bytes (complete=%d)\n", playbackOffset, current->complete);
...
void setup() {
  ...
  // Initialize RGB LED (RMT)
  if (!rmtInit(BUILTIN_RGBLED_PIN, RMT_TX_MODE, RMT_MEM_NUM_BLOCKS_1, 10000000)) {
    Serial.println("⚠️ RGB LED init failed");
  } else {
    setRgbLed(0,0,0); // off initially
  }
  ...
  // Download waiting music into PSRAM
  Serial.println("📥 Downloading waiting music...");
  if (waitingMusic.begin(WAIT_MUSIC_URL)) {
    Serial.println("✅ Waiting music loaded into PSRAM");
  } else {
    Serial.println("⚠️ Waiting music load failed");
  }
...
void loop() {
  ...
  if (lastButtonState == HIGH && currentButtonState == LOW) {
    ...
    if (!waiting_for_response) {
      ...
      // AGGRESSIVE CLEANUP: Clear previous audio and stop playback
      if (audioQueue.hasStreams() || isPlayingAudio) {
        Serial.println("🗑️ Clearing previous audio queue and stopping playback...");
        audioQueue.clear();
        reset_playback_state();
        
        // Give time for memory to be fully freed
        delay(100);
      }
      
      size_t free_psram = heap_caps_get_free_size(MALLOC_CAP_SPIRAM);
      Serial.printf("Free PSRAM: %d bytes (after clear)\n", free_psram);
      
      if (free_psram < 1200000) {
        Serial.println("⚠️ Low memory! Skipping...");
        return;
      }
      // ensure waiting music is stopped before recording
      waitingMusic.stop();
      digitalWrite(I2S_SD_SPK, LOW);
      ...
      Serial.println("⏳ Waiting for server response...");
        ...
        isPlayingAudio = false;
        // stop waiting music and start new recording
        waitingMusic.stop();
        digitalWrite(I2S_SD_SPK, LOW);
  ...
  // If waiting for server response and not playing server audio, loop waiting music
  if (waiting_for_response && !isPlayingAudio) {
    waitingMusic.loop();
  }
